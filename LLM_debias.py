...
# PROMPT_TEMPLATES = {
#     "book": (
#         "You are a book recommendation system now. "
#         'Please list the ranked recommendations. The output must be JSON like '
#         '{{"rank order":["A","B","C"...upto the last one]}}. '
#         "Input: Here is the reading history of a user: {history}. "
#         "The books on the candidate list are: {candidates}. Output:"
#     ),
#     "movie_lens": (
#         "You are a movie recommendation system now. "
#         'Return JSON {{"rank order":["A","B","C"... upto the last one]}} with the best viewing order. '
#         "Input: User has already watched: {history}. "
#         "Candidate movies: {candidates}. Output:"
#     ),
#     "news": (
#         "You are a news‑article recommendation engine. "
#         'Output JSON {{"rank order":["A","B","C"...upto the last one]}} where rank 0 is most relevant. '
#         "Input: Articles the user recently read: {history}. "
#         "Articles to choose from: {candidates}. Output:"
#     ),
# }
...
def build_prompt(
    ...
    def index_to_letter(index):
        if index < 26:
            ...
            # Handle cases beyond Z (AA, AB, AC, etc.)
            first_letter = chr(ord('A') + (index // 26) - 1)
            second_letter = chr(ord('A') + (index % 26))
            return first_letter + second_letter

    # Format user history as lettered list
    if user_history:
        history_str = "\n".join(f"{index_to_letter(i)}) {item}" for i, item in enumerate(user_history))
    else:
        history_str = "None"
    ...
    if candidate_list and isinstance(candidate_list[0], dict):
        ...
        candidate_str = "\n".join(
            f"{index_to_letter(i)}) {c['title']}" for i, c in enumerate(candidate_list)
        )
    else:
        # String format (backward compatibility)
        candidate_str = "\n".join(
            f"{index_to_letter(i)}) {c}" for i, c in enumerate(candidate_list)
        )

    # Generate dynamic example with shuffled letters from A to the last letter
    import random
    num_candidates = len(candidate_list)
    candidate_letters = [index_to_letter(i) for i in range(num_candidates)]
    random.shuffle(candidate_letters)
    # Show first few letters in the example
    example_letters = candidate_letters[:min(5, len(candidate_letters))]
    if len(candidate_letters) > 5:
        example_list = str(example_letters)[:-1] + ", ...]"  # Remove last ] and add ...
    else:
        example_list = str(example_letters)
    ...
    if dataset_type == "movie_lens":
        ...
        return (
            ...
            f'Return JSON with movie letters in order of preference. Example: {example_json}\n\n'
        ...
        return (
            ...
            f'Return JSON with song letters in order of preference. Do not miss any letter in the candidate list. Example: {example_json}\n\n'
        ...
        return (
            ...
            f'Return JSON with book letters in order of preference. Example: {example_json}\n\n'
        ...
        return (
            ...
            f'Return JSON with news letters in order of preference. Example: {example_json}\n\n'
        ...
        return (
            ...
            f'Return JSON with beauty product letters in order of preference. Example: {example_json}\n\n'
        ...
        return (
            ...
            f'Return JSON with steam game letters in order of preference. Example: {example_json}\n\n'
...
def call_model_for_ranking(prompt, model_name='gpt-3.5-turbo', backend='openai', model_api=None, custom_prompt=False):
        ...
        if json_match:
            ...
            try:
                ...
                if ranking_list and isinstance(ranking_list, list):
                    # Convert letters to 1-based indices
                    indices = []
                    for letter in ranking_list:
                        if isinstance(letter, str):
                            try:
                                index = letter_to_index(letter) + 1  # Convert to 1-based
                ...
                try:
                    # Look for array patterns like ["A", "B", "C"] or [A, B, C]
                    array_pattern = r'\[[^\]]*\]'
                    array_match = re.search(array_pattern, json_str)
                    if array_match:
                        array_str = array_match.group()
                        # Clean up the array string
                        array_str = re.sub(r'[^\w,\s\[\]]', '', array_str)
                        # Extract letters from the cleaned array
                        letters = re.findall(r'\b[A-Z]{1,2}\b', array_str)
                        if letters:
                            indices = []
    ...
    # Fallback: extract letters from the entire response
    try:
        letters = re.findall(r'\b[A-Z]{1,2}\b', content)  # Match A-Z and AA-ZZ patterns
        indices = []
        for letter in letters[:100]:  # Limit to first 100 matches
            try:
                index = letter_to_index(letter) + 1  # Convert to 1-based
                indices.append(index)
            except:
        ...
        # If we found some letters, return them
        if indices:
            return indices
    except:
        pass
...
class LLMPositionBiasAnalyzer:
    ...
    def _sequential_randomize_and_aggregate_with_raw_data(
        ...
        # [Previous aggregation code here - same as parallel version]
        aggregated_scores = {}
        aggregated_debiased_scores = {}
        avg_weights = {}
    ...
    def create_custom_propensity_scores(
        ...
        Args:
            N: Number of positions
            formula: Formula type ("inverse", "linear", "exponential", "custom")
            primacy_bias: Bias towards first items (for inverse formula)
            recency_bias: Bias towards last items (for inverse formula)
            middle_bias: Bias towards middle items (for inverse formula)
            custom_function: Custom function that takes position and returns weight

        Returns:
            Dictionary mapping position (0-based) to propensity weight
        """
        propensity_scores = {}

        if formula == "inverse" and primacy_bias is not None:
            # Use inverse propensity weighting based on bias
            for pos in range(N):
                if pos < int(0.25 * N):  # Primacy positions
                    weight = 1.0 / max(primacy_bias, 0.1)
                elif pos >= int(0.75 * N):  # Recency positions
            ...
            # Linear weighting (higher positions get higher weights)
            for pos in range(N):
                weight = 1.0 - (pos / (N - 1)) * 0.5  # Range from 1.0 to 0.5
                propensity_scores[pos] = weight

        elif formula == "exponential":
            # Exponential decay
            for pos in range(N):
                weight = np.exp(-pos / 10.0)  # Exponential decay
                propensity_scores[pos] = weight

        elif formula == "uniform":
            # Uniform weights (no debiasing)
            for pos in range(N):
    ...
    def reapply_debiasing_with_new_bias(
        ...
        This automatically calculates new propensity scores from the bias and recomputes metrics.

        Args:
            checkpoint_file: Path to checkpoint file with raw LLM data
            new_precalculated_bias: New bias scores (avg_primacy, avg_recency, avg_middle)
            aggregation_method: Aggregation method ("mean", "median", "max")
            save_results_to: Optional file to save recomputed results

        Returns:
            Dictionary with recomputed results using new bias-based propensity scores
        """
        ...
        # Get original experimental config to know num_candidates
        original_bias_analysis = checkpoint_data.get('bias_analysis', {})
        ...
        # Step 1: Calculate new propensity scores from the new bias
        ...
        # Step 2: Recompute all user results with new propensity scores
        ...
        # Step 4: Print results and comparison
        ...
        try:
            # Try to extract original metrics from the checkpoint
    ...
    def verify_bias_users_have_candidates(self, min_candidates: int = 10) -> bool:
        ...
        Returns:
            True if all bias users have sufficient candidates, False otherwise
        """
